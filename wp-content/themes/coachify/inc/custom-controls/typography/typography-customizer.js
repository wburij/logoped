( function( api ) {
	...
	api.controlConstructor['coachify-typography'] = api.Control.extend( {
		ready: function() {
			var control = this;

			control.container.on( 'change', '.coachify-font-family select',
				function() {
					...
					setTimeout( function() {
						// Send our request to the coachify_get_all_google_fonts_ajax function
						var response = jQuery.getJSON({
							type: 'POST',
							url: ajaxurl,
							data: {
								action: 'coachify_get_all_google_fonts_ajax',
								coachify_customize_nonce: coachify_customize.nonce
							},
							async: false,
							dataType: 'json',
						});
						...
						if ( id in fonts ) {
							...
							jQuery( '.coachify-font-family select' ).not( _this ).each( function( key, select ) {
								var parent = jQuery( this ).closest( '.coachify-font-family' );

								if ( _value == jQuery( select ).val() && _this.data( 'category' ) !== jQuery( select ).data( 'category' ) ) {
									if ( ! got_variants ) {
										updated_variants = jQuery( parent.next( '.coachify-font-variant' ).find( 'select' ) ).val();
										got_variants = true;
									}
								}
							} );
							...
							_this.closest( '.coachify-font-family' ).next( 'div' ).show();
							...
							_this.closest( '.coachify-font-family' ).next( 'div' ).hide();
			...
			control.container.on( 'change', '.coachify-font-variant select',
				function() {
					var _this = jQuery( this );
					...
					jQuery( '.coachify-font-variant select' ).each( function( key, value ) {
						var this_control = jQuery( this ).closest( 'li' ).attr( 'id' ).replace( 'customize-control-', '' );
						var parent = jQuery( this ).closest( '.coachify-font-variant' );
			...
			control.container.on( 'change', '.coachify-font-category input',
				function() {
					control.settings['category'].set( jQuery( this ).val() );
				}
			);

			control.container.on( 'change', '.coachify-font-weight select',
				function() {
					control.settings['weight'].set( jQuery( this ).val() );
				}
			);

			control.container.on( 'change', '.coachify-font-transform select',
				function() {
...
jQuery( document ).ready( function( $ ) {
	...
	$( '.coachify-font-family select' ).select2();

	$( '.coachify-font-variant' ).each( function( key, value ) {
		var _this = $( this );
	...
	$( ".coachify-font-family" ).each( function( key, value ) {
		var _this = $( this );
		if ( $.inArray( _this.find( 'select' ).val(), typography_defaults ) !== -1 ) {
			_this.next( '.coachify-font-variant' ).hide();
		}
	} );
