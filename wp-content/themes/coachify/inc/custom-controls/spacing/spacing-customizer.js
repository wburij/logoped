( function( $, api ) {
	...
	api.controlConstructor['spacing'] = api.Control.extend( {
		ready: function() {
			var control = this;
			$( '.coachify-number-control', control.container ).on( 'change keyup',
				function() {
	...
	api.controlConstructor['coachify-spacing-slider'] = api.Control.extend( {
		ready: function() {
			var control = this;
	...
	api.controlConstructor['coachify-spacing'] = api.Control.extend( {
		ready: function() {
			var control = this,
				controlClass = '.customize-control-coachify-spacing',
				footerActions = jQuery( '#customize-footer-actions' );

			// Figure out which device icon to make active on load
			jQuery( controlClass + ' .coachify-spacing-control-section' ).each( function() {
...
jQuery( document ).ready( function($) {
	$( '.coachify-link-spacing' ).on( 'click', function(e) {
		e.preventDefault();
		...
		_this.parent( '.coachify-spacing-section' ).prevAll().slice(0,4).find( 'input' ).addClass( 'linked-values' ).attr( 'data-element', element );
	...
	$( '.coachify-unlink-spacing' ).on( 'click', function(e) {
		e.preventDefault();
		...
		_this.parent( '.coachify-spacing-section' ).prevAll().slice(0,4).find( 'input' ).removeClass( 'linked-values' ).attr( 'data-element', '' );
	...
	$( '.coachify-spacing-section' ).on( 'input', '.linked-values', function() {
		var _this = $( this ),
