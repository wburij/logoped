wp.customize.controlConstructor['coachify-range-slider'] = wp.customize.Control.extend({
	...
	ready: function() {
		...
		var control = this,
		    value,
			controlClass = '.customize-control-coachify-range-slider',
			footerActions = jQuery( '#customize-footer-actions' );
		
		// Set up the sliders
		jQuery( '.coachify-slider' ).each( function() {
		...
		jQuery( controlClass + ' .coachify_range_value input[type=number]' ).on( 'input', function() {
			value = jQuery( this ).attr( 'value' );
			if ( '' == value ) {
				value = -1;
			}
			jQuery( this ).closest( 'label' ).find( '.coachify-slider' ).slider( 'value', parseFloat(value)).change();
		});

		// Handle the reset button
		jQuery( controlClass + ' .coachify-reset' ).on( 'click', function() {
			var icon = jQuery( this ),
				visible_area = icon.closest( '.coachify-range-title-area' ).next( '.coachify-range-slider-areas' ).children( 'label:visible' ),
				input = visible_area.find( 'input[type=number]' ),
				slider_value = visible_area.find( '.coachify-slider' ),
				visual_value = visible_area.find( '.coachify_range_value' ),
		...
		jQuery( controlClass + ' .coachify-range-slider-control' ).each( function() {
			var _this = jQuery( this );
			_this.find( '.coachify-device-controls' ).children( 'span:first-child' ).addClass( 'selected' );
			_this.find( '.range-option-area:first-child' ).show();
		});
		
		// Do stuff when device icons are clicked
		jQuery( controlClass + ' .coachify-device-controls > span' ).on( 'click', function( event ) {
			var device = jQuery( this ).data( 'option' );
			
			jQuery( controlClass + ' .coachify-device-controls span' ).each( function() {
				var _this = jQuery( this );
			...
			jQuery( controlClass + ' .coachify-range-slider-areas label' ).each( function() {
				var _this = jQuery( this );
				if ( device == _this.attr( 'data-option' ) ) {
					_this.show();
					_this.siblings().hide();
